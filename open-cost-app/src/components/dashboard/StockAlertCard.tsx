...
export function StockAlertCard({ noWrapper = false }: StockAlertCardProps) {
    ...
    if (loading) {
        if (noWrapper) return <div className="p-4 text-center text-xs font-bold animate-pulse text-slate-400">데이터 로딩 중...</div>
        return (
    ...
    return (
        <Card className="col-span-1 md:col-span-2 lg:col-span-3 border-none shadow-none bg-orange-500/5">
            <CardHeader className="pb-2">
                ...
                {!noWrapper && (
                    <CardDescription className="text-orange-500/80 font-bold text-xs">
                        안전 재고 수준 미만입니다. 발주가 필요합니다.
                    </CardDescription>
                )}
            </CardHeader>
            {content}
